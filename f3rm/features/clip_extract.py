...
class Extractor:
    def __init__(self, model_name):
        ...
        print(f"Loaded CLIP model for pixel aligned {model_name}")
    ...
    @torch.no_grad()
    def extract_clip_features(self, image: Image.Image, device: torch.device) -> torch.Tensor:
        """Extract dense patch-level CLIP features for given images"""

        # Preprocess the images
        preprocessed_images = torch.stack([self.preprocess(image)])
        ...
        if self.model_name.startswith("ViT"):
        ...
        elif self.model_name.startswith("RN"):
            ...
            raise ValueError(f"Unknown CLIP model name: {self.model_name}")
